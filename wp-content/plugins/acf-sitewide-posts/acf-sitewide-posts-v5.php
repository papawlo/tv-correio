...
class acf_field_sitewide_posts extends acf_field {
	...
	*/
	...
	function __construct() {
		...
		// extra
		add_action('wp_ajax_acf/fields/sitewide_posts/query',			array($this, 'ajax_query'));
		add_action('wp_ajax_nopriv_acf/fields/sitewide_posts/query',	array($this, 'ajax_query'));
	...
	*/
	...
	function get_choices( $options = array() ) {
		...
		// filters
		$args = apply_filters('acf/fields/sitewide_posts/query', $args, $field, $options['post_id']);
		$args = apply_filters('acf/fields/sitewide_posts/query/name=' . $field['name'], $args, $field, $options['post_id'] );
		$args = apply_filters('acf/fields/sitewide_posts/query/key=' . $field['key'], $args, $field, $options['post_id'] );
	...
	*/
	...
	function get_post_title( $post, $field, $post_id = 0 ) {
		...
		// filters
		$title = apply_filters('acf/fields/sitewide_posts/result', $title, $post, $field, $post_id);
		$title = apply_filters('acf/fields/sitewide_posts/result/name=' . $field['_name'], $title, $post, $field, $post_id);
		$title = apply_filters('acf/fields/sitewide_posts/result/key=' . $field['key'], $title, $post, $field, $post_id);
	...
	*/
	...
	function render_field( $field ) {
		...
		$atts = array(
			'id'				=> $field['id'],
			'class'				=> "acf-sitewide_posts {$field['class']}",
			'data-max'			=> $field['max'],
			'data-s'			=> '',
			'data-post_type'	=> '',
			'data-taxonomy'		=> '',
