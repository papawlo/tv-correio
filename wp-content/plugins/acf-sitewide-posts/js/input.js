(function($){
	...
	acf.fields.sitewide_posts = acf.field.extend({
		...
		focus: function(){
			...
			this.$el = this.$field.find('.acf-sitewide_posts');
...
*/
		...
		fetch: function(){
			...
			var data = acf.prepare_for_ajax({
				action:		'acf/fields/sitewide_posts/query',
			...
			// add message
			this.$choices.children('.list').append('<p>' + acf._e('sitewide_posts', 'loading') + '...</p>');
		...
		render: function( json ){
			...
			if( !json || !json.length ) {
				...
				if( this.settings.paged == 1 ) {
					...
					this.$choices.children('.list').append('<p>' + acf._e('sitewide_posts', 'empty') + '</p>');
		...
		add_item: function( e ){
			...
			if( this.settings.max > 0 ) {
				...
				if( this.$values.find('.acf-rel-item').length >= this.settings.max ) {
					...
					alert( acf._e('sitewide_posts', 'max').replace('{max}', this.settings.max) );
